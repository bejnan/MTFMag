...
shared_ptr<vector< DataCollector::DataInputLine> > DataCollector::ReadInputLine(int turn_amount) {
  string line;
  stringstream input_line_stream;
  vector<DataCollector::DataInputLine> * input_lines_data = new vector<DataCollector::DataInputLine>();
  ...
  input_lines_data->push_back(DataCollector::DataInputLine(interaction, timestamp, sender_id,
                                        receiver_id));
  }
